...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelector('#app').innerHTML = `
        <nav>
            <div class="logo" role="img" alt="Quick Notepad">
            <div class="mark"></div>
            <div class="type">tinyNotepad</div>
            </div>
            <button class="mode-switch"></button>
        </nav>
        <label class="visually-hidden" for="text-area">Notepad</label>
        <div id="notepad-container" contenteditable="true"></div>
    ...
    notepadContainer.focus();

    try {
        const notepad = JSON.parse(localStorage.getItem('notepad'));
        notepadContainer.innerHTML = notepad.text;
        notepadContainer.style.height = notepad.height;
        charCount.innerHTML = `${notepadContainer.textContent.length}`;
    ...
    function saveNotepad() {
        storage.setItem('notepad', JSON.stringify({
            text: notepadContainer.innerHTML,
            height: (notepadContainer.style.height = `${notepadContainer.scrollHeight}px`),
            darkMode: isDarkMode,
        }));
    }

    document.addEventListener('input', (e) => {
        notepadContainer.style.height = `${notepadContainer.scrollHeight}px`;
        charCount.innerHTML = `${notepadContainer.textContent.length}`;

        debounce(saveNotepad);
    });

    function changeMode(dark = false, save = true) {
        if (dark) {
            body.classList.add('dark');
            switchMode.innerHTML = sunIcon;  
        } else {
